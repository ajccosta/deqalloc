...
#ifndef DQ_SEGMENTHEAP_H
#define DQ_SEGMENTHEAP_H

#include "deqalloc/heaps/top/mmapheap.h"
#include "deqalloc/heaps/combining/minisegheap.h"
#include "threads/cpuinfo.h"
#include "deqalloc/threads/epoch.h"
#include "deqalloc/threads/structures/HarrisLinkedList.h"
...
class SegmentHeap : public deqalloc::AlignedMmapHeap {
  public:
    static constexpr size_t SegmentSize = SegmentSize_; //Make SegmentSize visible to other classes
    enum { Alignment = SegmentSize };

    static_assert(SegmentSize % HL::CPUInfo::PageSize == 0 && "SegmentSize must be multiple of page size");
...
pushNode_n_retry:
  ...
  public:
    ...
    static constexpr size_t roundToPageSize(size_t sz) {
      return (sz + HL::CPUInfo::PageSize - 1) & (size_t) ~(HL::CPUInfo::PageSize - 1);
    }
    ...
    header_t* allocateSegment(size_t sz) {
      ...
      if(getActualSegmentSize(sz) == SegmentSize && segment_pool != nullptr) {
        ...
        header = static_cast<header_t*>(deqalloc::AlignedMmapHeap::malloc(getActualSegmentSize(sz), SegmentSize));
    ...
    void freeSegment(header_t* header) {
      deqalloc::AlignedMmapHeap::free(header, getActualSegmentSize(header->getSize()));
    }
