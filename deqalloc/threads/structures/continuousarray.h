#ifndef CONTINUOUSARRAY_H
#define CONTINUOUSARRAY_H
...
//TODO: At the moment, blocks are only reused (never free'd). Add heuristic to free some blocks
  ...
  private:
    ...
    struct alignas(64) block {
      ...
      static constexpr bool is_tuple = is_tuple_v<T>;
      using T_ = std::conditional_t<is_tuple, atomic_tuple<T>, std::atomic<T>>;
      T_ array[block_size];
    };
    ...
    //TODO: FIX BUG
    // if two threads calling ca_retire_last_block concurrently
    // read the same tail value into old_block, only 1 block will be popped,
    // where the correct value has to be exactly 1 block is popped per call
    void
